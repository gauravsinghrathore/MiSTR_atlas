...
```{r}
d9d.data <- Read10X("/nfsdata/projects/gaurav/c5-exo/data-runs/170907-kirkeby-mistr/d9D_2-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
d9d.hto.outs <- Read10X("/nfsdata/data/data-citeseq/outs/d9D/umi_count/",gene.column=1)
d9d.hto.outs <- d9d.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d9d.data), colnames(d9d.hto.outs))

d9d.data <- d9d.data[, joint.bcs]
d9d.hto.outs <- as.matrix(d9d.hto.outs[, joint.bcs])
...
d9d <- CreateSeuratObject(d9d.data,project = "d9d")
d9d <- Seurat.NormAndScale(d9d)
```
...
d9d[["HTO"]] <- CreateAssayObject(counts = d9d.hto.outs)

d9d <- NormalizeData(d9d, assay = "HTO", normalization.method = "CLR")
head(rownames(d9d[["HTO"]]))
```
...
```{r}
d9d <- HTODemux(d9d, assay = "HTO", positive.quantile = 0.99)
table(d9d$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d9d) <- "HTO_classification.global"
d9d.fp <- subset(d9d, idents = c("Singlet","Negative"))
```


```{r}
d9d.fp[["percent.mt"]] <- PercentageFeatureSet(d9d.fp, pattern = "^MT-")
d9d.fp <- subset(d9d.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 8000 & percent.mt < 15)
d9d.fp <- Filter_Mito_Ribo(d9d.fp)


#regress out umi noise ##########

#d9d.fp <- SCTransform(d9d.fp, vars.to.regress = "nCount_RNA", verbose = FALSE)

#######normalise#####
d9d.fp <- Seurat.NormAndScale(d9d.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d9d.fp <- seurat.cellcycle(d9d.fp)

saveRDS(d9d.fp, file = "~/new_analysis_new_cc/rds/d9d.rds")
```
