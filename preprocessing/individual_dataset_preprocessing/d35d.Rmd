...
```{r}
d35d.data <- Read10X("/nfsdata/projects/gaurav/c5-exo/data-runs/170907-kirkeby-mistr/d35D-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
d35d.hto.outs <- Read10X("/nfsdata/data/data-citeseq/outs/d35D/umi_count/",gene.column=1)
d35d.hto.outs <- d35d.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d35d.data), colnames(d35d.hto.outs))

d35d.data <- d35d.data[, joint.bcs]
d35d.hto.outs <- as.matrix(d35d.hto.outs[, joint.bcs])
...
d35d <- CreateSeuratObject(d35d.data,project = "d35d")
d35d <- Seurat.NormAndScale(d35d)
```
...
d35d[["HTO"]] <- CreateAssayObject(counts = d35d.hto.outs)

d35d <- NormalizeData(d35d, assay = "HTO", normalization.method = "CLR")
head(rownames(d35d[["HTO"]]))
```
...
```{r}
d35d <- HTODemux(d35d, assay = "HTO", positive.quantile = 0.99)
table(d35d$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d35d) <- "HTO_classification.global"
d35d.fp <- subset(d35d, idents = c("Singlet","Negative"))
```


```{r}
d35d.fp[["percent.mt"]] <- PercentageFeatureSet(d35d.fp, pattern = "^MT-")
d35d.fp <- subset(d35d.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 6500 & percent.mt < 15)
d35d.fp <- Filter_Mito_Ribo(d35d.fp)



#######normalise#####
d35d.fp <- Seurat.NormAndScale(d35d.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d35d.fp <- seurat.cellcycle(d35d.fp)

saveRDS(d35d.fp, file = "~/new_analysis_new_cc/rds/d35d.rds")
```
