...
```{r}
d62d.data <- Read10X("/nfsdata/projects/gaurav/c5-exo/data-runs/170907-kirkeby-mistr/d62D-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
d62d.hto.outs <- Read10X("/nfsdata/data/data-citeseq/outs/d62D/umi_count/",gene.column=1)
d62d.hto.outs <- d62d.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d62d.data), colnames(d62d.hto.outs))

d62d.data <- d62d.data[, joint.bcs]
d62d.hto.outs <- as.matrix(d62d.hto.outs[, joint.bcs])
...
d62d <- CreateSeuratObject(d62d.data,project = "d62d")
d62d <- Seurat.NormAndScale(d62d)
```
...
d62d[["HTO"]] <- CreateAssayObject(counts = d62d.hto.outs)

d62d <- NormalizeData(d62d, assay = "HTO", normalization.method = "CLR")
head(rownames(d62d[["HTO"]]))
```
...
```{r}
d62d <- HTODemux(d62d, assay = "HTO", positive.quantile = 0.99)
table(d62d$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d62d) <- "HTO_classification.global"
d62d.fp <- subset(d62d, idents = c("Singlet","Negative"))
```


```{r}
d62d.fp[["percent.mt"]] <- PercentageFeatureSet(d62d.fp, pattern = "^MT-")
d62d.fp <- subset(d62d.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 4500 & percent.mt < 15)
d62d.fp <- Filter_Mito_Ribo(d62d.fp)



#######normalise#####
d62d.fp <- Seurat.NormAndScale(d62d.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d62d.fp <- seurat.cellcycle(d62d.fp)

saveRDS(d62d.fp, file = "~/new_analysis_new_cc/rds/d62d.rds")
```
