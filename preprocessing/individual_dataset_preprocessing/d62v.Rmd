...
```{r}
d62v.data <- Read10X("/nfsdata/projects/gaurav/c5-exo/data-runs/170907-kirkeby-mistr/d62V-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
d62v.hto.outs <- Read10X("/nfsdata/data/data-citeseq/outs/d62V/umi_count/",gene.column=1)
d62v.hto.outs <- d62v.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d62v.data), colnames(d62v.hto.outs))

d62v.data <- d62v.data[, joint.bcs]
d62v.hto.outs <- as.matrix(d62v.hto.outs[, joint.bcs])
...
d62v <- CreateSeuratObject(d62v.data,project = "d62v")
d62v <- Seurat.NormAndScale(d62v)
```
...
d62v[["HTO"]] <- CreateAssayObject(counts = d62v.hto.outs)

d62v <- NormalizeData(d62v, assay = "HTO", normalization.method = "CLR")
head(rownames(d62v[["HTO"]]))
```
...
```{r}
d62v <- HTODemux(d62v, assay = "HTO", positive.quantile = 0.99)
table(d62v$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d62v) <- "HTO_classification.global"
d62v.fp <- subset(d62v, idents = c("Singlet","Negative"))
```


```{r}
d62v.fp[["percent.mt"]] <- PercentageFeatureSet(d62v.fp, pattern = "^MT-")
d62v.fp <- subset(d62v.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 4500 & percent.mt < 15)
d62v.fp <- Filter_Mito_Ribo(d62v.fp)



#######normalise#####
d62v.fp <- Seurat.NormAndScale(d62v.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d62v.fp <- seurat.cellcycle(d62v.fp)

saveRDS(d62v.fp, file = "~/new_analysis_new_cc/rds/d62v.rds")
```
