...
```{r}
d35dv.data <- Read10X("/nfsdata/projects/gaurav/fatima_data/count_metrices/35d/filtered_feature_bc_matrix/",strip.suffix = T)
d35dv.hto.outs <- Read10X("/nfsdata/projects/gaurav/fatima_data/count_metrices/35d/umi_count/",gene.column=1)
d35dv.hto.outs <- d35dv.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d35dv.data), colnames(d35dv.hto.outs))

d35dv.data <- d35dv.data[, joint.bcs]
d35dv.hto.outs <- as.matrix(d35dv.hto.outs[, joint.bcs])
...
d35dv <- CreateSeuratObject(d35dv.data,project = "d35dv")
d35dv <- Seurat.NormAndScale(d35dv)
```
...
d35dv[["HTO"]] <- CreateAssayObject(counts = d35dv.hto.outs)

d35dv <- NormalizeData(d35dv, assay = "HTO", normalization.method = "CLR")
head(rownames(d35dv[["HTO"]]))
```
...
```{r}
d35dv <- HTODemux(d35dv, assay = "HTO", positive.quantile = 0.99)
table(d35dv$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d35dv) <- "HTO_classification.global"
d35dv.fp <- subset(d35dv, idents = c("Singlet","Negative"))
```


```{r}
d35dv.fp[["percent.mt"]] <- PercentageFeatureSet(d35dv.fp, pattern = "^MT-")
d35dv.fp <- Filter_Mito_Ribo(d35dv.fp)
d35dv.fp <- subset(d35dv.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 6000 & percent.mt < 15)


#######normalise#####
d35dv.fp <- Seurat.NormAndScale(d35dv.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d35dv.fp <- seurat.cellcycle(d35dv.fp)

saveRDS(d35dv.fp, file = "~/DV_mistr/rds/d35dv.rds")
```
