...
```{r}
d1d.data <- Read10X("/nfsdata/data/data-runs/mistr_cellranger5.0/170907-kirkeby-mistr/d1d-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
...
d1d <- CreateSeuratObject(d1d.data,project = "d1d")
...
```{r}
d1d[["percent.mt"]] <- PercentageFeatureSet(d1d, pattern = "^MT-")
d1d <- subset(d1d, subset = nFeature_RNA > 200 & nFeature_RNA < 6500 & percent.mt < 15)
d1d <- Filter_Mito_Ribo(d1d)
...
```{r}
saveRDS(d1d, file = "~/new_analysis_new_cc/rds/d1d.rds")
```
