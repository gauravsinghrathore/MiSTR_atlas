...
```{r}
d21dv.data <- Read10X("/nfsdata/projects/gaurav/fatima_data/count_metrices/21d/filtered_feature_bc_matrix/",strip.suffix = T)
d21dv.hto.outs <- Read10X("/nfsdata/projects/gaurav/fatima_data/count_metrices/21d/umi_count/",gene.column=1)
d21dv.hto.outs <- d21dv.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d21dv.data), colnames(d21dv.hto.outs))

d21dv.data <- d21dv.data[, joint.bcs]
d21dv.hto.outs <- as.matrix(d21dv.hto.outs[, joint.bcs])
...
d21dv <- CreateSeuratObject(d21dv.data,project = "d21dv")
d21dv <- Seurat.NormAndScale(d21dv)
```
...
d21dv[["HTO"]] <- CreateAssayObject(counts = d21dv.hto.outs)

d21dv <- NormalizeData(d21dv, assay = "HTO", normalization.method = "CLR")
head(rownames(d21dv[["HTO"]]))
```
...
```{r}
d21dv <- HTODemux(d21dv, assay = "HTO", positive.quantile = 0.99)
table(d21dv$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d21dv) <- "HTO_classification.global"
d21dv.fp <- subset(d21dv, idents = c("Singlet","Negative"))
```


```{r}
d21dv.fp[["percent.mt"]] <- PercentageFeatureSet(d21dv.fp, pattern = "^MT-")
d21dv.fp <- Filter_Mito_Ribo(d21dv.fp)
d21dv.fp <- subset(d21dv.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 6000 & percent.mt < 15)


#######normalise#####
d21dv.fp <- Seurat.NormAndScale(d21dv.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d21dv.fp <- seurat.cellcycle(d21dv.fp)

saveRDS(d21dv.fp, file = "~/DV_mistr/rds/d21dv.rds")
```
