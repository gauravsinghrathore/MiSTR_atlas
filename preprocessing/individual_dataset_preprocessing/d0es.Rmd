...
```{r}
d0es.data <- Read10X("/nfsdata/data/data-runs/mistr_cellranger5.0/170907-kirkeby-mistr/d0es-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
...
d0es <- CreateSeuratObject(d0es.data,project = "d0es")
...
```{r}
d0es[["percent.mt"]] <- PercentageFeatureSet(d0es, pattern = "^MT-")
d0es <- subset(d0es, subset = nFeature_RNA > 200 & nFeature_RNA < 6000 & percent.mt < 15)
d0es <- Filter_Mito_Ribo(d0es)
...
```{r}
saveRDS(d0es, file = "~/new_analysis_new_cc/rds/d0es.rds")
```
