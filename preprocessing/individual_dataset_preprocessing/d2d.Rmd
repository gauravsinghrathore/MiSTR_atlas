...
```{r}
d2d.data <- Read10X("/nfsdata/data/data-runs/mistr_cellranger5.0/170907-kirkeby-mistr/d2D-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
...
d2d <- CreateSeuratObject(d2d.data,project = "d2d")
...
```{r}
d2d[["percent.mt"]] <- PercentageFeatureSet(d2d, pattern = "^MT-")
d2d <- subset(d2d, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mt < 10)
d2d <- Filter_Mito_Ribo(d2d)
...
```{r}
saveRDS(d2d, file = "~/new_analysis_new_cc/rds/d2d.rds")
```
