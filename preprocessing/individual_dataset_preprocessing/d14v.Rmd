...
```{r}
#d14v.data <- Read10X("/nfsdata/data/data-runs/mistr_cellranger5.0/170907-kirkeby-mistr/d14v-5000_cells/outs/filtered_feature_bc_matrix/", strip.suffix = T)

########create whitelist#######
#umi_counts<-Matrix::colSums(d14v.data)
#d14v.data<-d14v.data[,order(-umi_counts)]
#d14v_whitelist<-colnames(d14v.data)[1:20000]
#write.table(d14v_whitelist, file = "/nfsdata/data/data-citeseq/whitelist/d14v_whitelist", quote=F,  eol = "\n", row.names=F)
...
```{r}
d14v.data <- Read10X("/nfsdata/data/data-runs/mistr_cellranger5.0/170907-kirkeby-mistr/d14V-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
d14v.hto.outs <- Read10X("/nfsdata/data/data-citeseq/outs/d14V/umi_count/",gene.column=1)
...
d14v.fp <- subset(d14v, idents = c("Singlet","Negative"))
```


```{r}
d14v.fp[["percent.mt"]] <- PercentageFeatureSet(d14v.fp, pattern = "^MT-")
d14v.fp <- Filter_Mito_Ribo(d14v.fp)
d14v.fp <- subset(d14v.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 7000 & percent.mt < 15)


#######normalise#####
d14v.fp <- Seurat.NormAndScale(d14v.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d14v.fp <- seurat.cellcycle(d14v.fp)

saveRDS(d14v.fp, file = "~/new_analysis_new_cc/rds/d14v.rds")
```
