...
```{r}
d21d.data <- Read10X("/nfsdata/projects/gaurav/c5-exo/data-runs/170907-kirkeby-mistr/d21D-5000_cells/outs/filtered_feature_bc_matrix/",strip.suffix = T)
d21d.hto.outs <- Read10X("/nfsdata/data/data-citeseq/outs/d21D/umi_count/",gene.column=1)
d21d.hto.outs <- d21d.hto.outs[-6,]
...
joint.bcs <- intersect(colnames(d21d.data), colnames(d21d.hto.outs))

d21d.data <- d21d.data[, joint.bcs]
d21d.hto.outs <- as.matrix(d21d.hto.outs[, joint.bcs])
...
d21d <- CreateSeuratObject(d21d.data,project = "d21d")
d21d <- Seurat.NormAndScale(d21d)
```
...
d21d[["HTO"]] <- CreateAssayObject(counts = d21d.hto.outs)

d21d <- NormalizeData(d21d, assay = "HTO", normalization.method = "CLR")
head(rownames(d21d[["HTO"]]))
```
...
```{r}
d21d <- HTODemux(d21d, assay = "HTO", positive.quantile = 0.99)
table(d21d$HTO_classification.global)
```
isolates fps as a new seurat object

```{r}
Idents(d21d) <- "HTO_classification.global"
d21d.fp <- subset(d21d, idents = c("Singlet","Negative"))
```


```{r}
d21d.fp[["percent.mt"]] <- PercentageFeatureSet(d21d.fp, pattern = "^MT-")
d21d.fp <- subset(d21d.fp, subset = nFeature_RNA > 200 & nFeature_RNA < 8000 & percent.mt < 15)
d21d.fp <- Filter_Mito_Ribo(d21d.fp)



#######normalise#####
d21d.fp <- Seurat.NormAndScale(d21d.fp)

########cell cycle and umi regression######
#library(future)
#plan("multiprocess", workers = 80)
#options(future.globals.maxSize= 10000 * 1024^2)
d21d.fp <- seurat.cellcycle(d21d.fp)

saveRDS(d21d.fp, file = "~/new_analysis_new_cc/rds/d21d.rds")
```
